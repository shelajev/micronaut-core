...
==== Before building the native image using Docker

Building the native image works on the JVM bytecode, so one needs to build the application before trying to convert it the the GraalVM native image. To do it, please ensure that you've followed the [getting started](docs/guide/languageSupport/graal.adoc#getting-started) process. 
...
[source,bash]
----
$ docker build . -t hello-world # <0>
$ docker run --network=host hello-world # <1>
----
<0> If the `RUN native-image` step fails with error code 137 it might be that your Docker instance doesn't have enough memory for the AOT compilation. Please increase the [memory available to Docker](https://docs.docker.com/config/containers/resource_constraints/#limit-a-containers-access-to-memory). 
